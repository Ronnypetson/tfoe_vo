...
def plot_dense_depth(im0, im1, T_acc, foe, c_tc):
    ...
    #d = np.log(d + 1e-10)
    d = np.clip(d, 0, np.mean(d) + 1 * np.std(d))
    d = (255 * d / np.max(d)).astype(np.uint8)
    d = np.clip(d, 0, 220)
    #d = cv2.applyColorMap(d, cv2.COLORMAP_HSV)
    #cv2.rectangle(d, (foe[0] - 5, foe[1] - 5), (foe[0] + 5, foe[1] + 5), (0, 0, 0), 2)

    #im1 = np.reshape(im1, (h, w, 1))
    #im1 = np.concatenate([im1, im1, im1], axis=-1)
    #cv2.imwrite('none/test_depth.png', 0.5 * d + 0.5 * im1)
