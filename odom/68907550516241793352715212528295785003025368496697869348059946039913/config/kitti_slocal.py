    ...
    try:
        for i in range(0, kp.seq_len - (baw - 1), baw - 1):
            ...
            if show_cloud:
                ...
                velo = np.array([x for x in velo if x[-1] > 0])
                velo[:, -1] = 1
                ...
                # Draw the points
                im0 = Image.fromarray(np.uint8(cm.gray(im0) * 255))
                #im0 = Image.fromarray(np.uint8(im0) * 255)
                img_draw = ImageDraw.Draw(im0)
                for xz, x in zip(vd, velo):
                    py, px = x[:2]
                    #img_draw.point(x[:2], fill=(255 - xz, 0, xz))
                    img_draw.rectangle([px - 1, py - 1, px + 1, py + 1],
                                       fill=(255 - xz, 0, xz),
                                       width=2)
                im0.show()
